...
#+HTML:<img src="https://github.com/KcMeterCEC/explore/blob/master/[goodBook]/[What]goodBook--ostep/virtualization/mempic/swap/AMAT.jpg?raw=true" alt="mempic/swap/AMAT.jpg">
- TM : 访问内存消耗时间
- TD : 访问硬盘消耗时间
...
#+HTML:<img src="https://github.com/KcMeterCEC/explore/blob/master/[goodBook]/[What]goodBook--ostep/virtualization/mempic/swap/policy_cmp.jpg?raw=true" alt="mempic/swap/policy_cmp.jpg">
可以看到，在随机访问页面的情况下：
...
#+HTML:<img src="https://github.com/KcMeterCEC/explore/blob/master/[goodBook]/[What]goodBook--ostep/virtualization/mempic/swap/policy_cmp2.jpg?raw=true" alt="mempic/swap/policy_cmp2.jpg">
可以看到，在有规律的访问页面的情况下：
...
#+HTML:<img src="https://github.com/KcMeterCEC/explore/blob/master/[goodBook]/[What]goodBook--ostep/virtualization/mempic/swap/policy_cmp3.jpg?raw=true" alt="mempic/swap/policy_cmp3.jpg">
可以看到：
...
#+HTML:<img src="https://github.com/KcMeterCEC/explore/blob/master/[goodBook]/[What]goodBook--ostep/virtualization/mempic/swap/policy_cmp4.jpg?raw=true" alt="mempic/swap/policy_cmp4.jpg">
上图可以看出这种近似 LRU 算法的效率接近理论上的 LRU 算法。
* 考虑脏页
当进行置换时，如果当前的内存页已经被写过了，那么需要将其内容写回至硬盘，这种 I/O 操作比较耗时。
如果页没有被修改过，那么并不需要实际的 I/O 操作，这种置换就相对省时得多。
- 但如果一个页面不是 file backend，并且在交换空间中没有备份，那么即使该页面没有被修改过，也会在置换该页面时发生 I/O 操作。
- 在页表项中就具有一位 =modified bit= (dirty bit) 来表示该页是否被写过

那么对于置换策略来讲，当然是在最近最少使用的页面中，选出那些没有被修改过的页面来置换，这样效率才最高。
* 其他策略
目前只讲到了基本的交换策略，还有其他很多方面的策略：
- 页回读：系统将硬盘上的页面回读到内存的策略
  + 比如当一个页面被访问，那么系统会预读多个页面以提高效率
- 页回写：系统什么时候将被修改过的页面写回硬盘
  + 一般是将多个写请求合并为一个请求然后一次性的写入以提高效率
